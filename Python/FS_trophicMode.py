...
def main():
	parser = argparse.ArgumentParser(
		description= "Code to:\n" +
		"Feature Selection on trophicMode")
	...
	print("=======>>>>>>>>>>>>> Start to run trophicMode feature selction ....")
	trophicMode_EFS = EnsembleFeatureSelection(input_feature_data_name=feature_data_name,
	...
	trophicMode_SelectedFeatureNameInformation = trophicMode_EFS.get_result(input_X_array= X_train_array,
																			input_y_array= trophicMode_train_array,
																			input_y_array_unknown_index= na_index_train_Info['trophicMode'],
																			input_y_name= label_Info['trophicMode'])
	with open(feature_selection_path+feature_data_name+"--"+'trophicMode'+"--feature_selection_list"+"--"+Tag+".txt","w") as file:
		for i in trophicMode_SelectedFeatureNameInformation.keys():
			feature_string = ",".join(trophicMode_SelectedFeatureNameInformation[i])
			file.write("%s\t" % i)
			file.write("%s\n" % feature_string)


	selected_list = []
	for _,k in trophicMode_SelectedFeatureNameInformation.items():
